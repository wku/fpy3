...
#ifdef REQUEST_OPAQUE
#define REQUEST_FREELIST_DEFAULT_MAX 1024
static PyObject **request_freelist = NULL;
static int request_freelist_len = 0;
static int request_freelist_max = REQUEST_FREELIST_DEFAULT_MAX;

static PyObject*
Request_configure_pool(PyObject* self, PyObject* args, PyObject* kwds)
{
    int max_size = REQUEST_FREELIST_DEFAULT_MAX;
    ...
    if (max_size < 0) {
        PyErr_SetString(PyExc_ValueError, "max_size must be positive");
        return NULL;
    }

    if (request_freelist_max == max_size && request_freelist != NULL)
        Py_RETURN_NONE;

    // Free excess items if shrinking
    while (request_freelist_len > max_size) {
       PyObject* item = request_freelist[--request_freelist_len];
       Py_TYPE(item)->tp_free(item);
    }

    if (max_size == 0) {
        free(request_freelist);
        request_freelist = NULL;
    } else {
        PyObject **new_freelist = realloc(request_freelist, sizeof(PyObject*) * max_size);
        if (!new_freelist) {
            return PyErr_NoMemory();
        }
        request_freelist = new_freelist;
    }

    request_freelist_max = max_size;
...
#endif
{
  ...
  if (request_freelist && request_freelist_len > 0) {
    self = (Request*)request_freelist[--request_freelist_len];
...
#ifdef REQUEST_OPAQUE
  if (request_freelist && request_freelist_len < request_freelist_max) {
    request_freelist[request_freelist_len++] = (PyObject*)self;
  } else {
...
static PyMethodDef crequest_methods[] = {
  {"configure_pool", (PyCFunction)Request_configure_pool, METH_VARARGS | METH_KEYWORDS,
   "Configure the request memory pool size."},
  {NULL, NULL, 0, NULL}
};
...
static PyModuleDef crequest = {
  ...
  "crequest",
  -1,
  crequest_methods, NULL, NULL, NULL, NULL
};
#endif
...
#define alloc_static2(name, val) \
  ...
  cresponse = PyImport_ImportModule("fpy3.response.cresponse");
  if(!cresponse)
    goto error;
...
#ifdef REQUEST_OPAQUE
  request = PyImport_ImportModule("fpy3.request");
  if(!request)
    goto error;
  ...
  api_capsule = export_capi(m, "fpy3.request.crequest", &capi);
  if(!api_capsule)
    goto error;

#endif
  response_capi = import_capi("fpy3.response.cresponse");
  if(!response_capi)
    goto error;
