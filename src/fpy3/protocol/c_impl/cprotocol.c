...
Protocol_connection_made(Protocol* self, PyObject* transport)
{
  ...
  if(!(fileno_res = PyObject_CallMethod(socket, "fileno", NULL)))
    goto error;

  sock_fd = (int)PyLong_AsLong(fileno_res);
...
Bytes_FromSize(size_t size)
{
  ...
  Py_SET_TYPE(result, &PyBytes_Type);
...
PyObject* Gather_flush(Gather* gather)
{
  ...
  if(gather->prev_buffer) {
    if(Py_REFCNT(gather->prev_buffer) == 1) {
      ...
      Py_SET_SIZE(gather_buffer, (ssize_t)gather->len);
    } else {
...
PyInit_cprotocol(void)
{
...
#ifdef PARSER_STANDALONE
  cparser = PyImport_ImportModule("fpy3.parser.cparser");
  if(!cparser)
    goto error;
  ...
  crequest = PyImport_ImportModule("fpy3.request.crequest");
  if(!crequest)
    goto error;
  ...
  if(!(route = PyImport_ImportModule("fpy3.router.route")))
    goto error;
  ...
  request_capi = import_capi("fpy3.request.crequest");
  if(!request_capi)
    goto error;

  matcher_capi = import_capi("fpy3.router.cmatcher");
  if(!matcher_capi)
    goto error;
  ...
  api_capsule = export_capi(m, "fpy3.protocol.cprotocol", &capi);
  if(!api_capsule)
    goto error;
