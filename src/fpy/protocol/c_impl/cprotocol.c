...
PyInit_cprotocol(void)
{
...
#ifdef PARSER_STANDALONE
  cparser = PyImport_ImportModule("fpy.parser.cparser");
  if(!cparser)
    goto error;
  ...
  crequest = PyImport_ImportModule("fpy.request.crequest");
  if(!crequest)
    goto error;
  ...
  if(!(route = PyImport_ImportModule("fpy.router.route")))
    goto error;
  ...
  request_capi = import_capi("fpy.request.crequest");
  if(!request_capi)
    goto error;

  matcher_capi = import_capi("fpy.router.cmatcher");
  if(!matcher_capi)
    goto error;
  ...
  api_capsule = export_capi(m, "fpy.protocol.cprotocol", &capi);
  if(!api_capsule)
    goto error;
