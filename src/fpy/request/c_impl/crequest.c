...
#define alloc_static2(name, val) \
  ...
  cresponse = PyImport_ImportModule("fpy.response.cresponse");
  if(!cresponse)
    goto error;
...
#ifdef REQUEST_OPAQUE
  request = PyImport_ImportModule("fpy.request");
  if(!request)
    goto error;
  ...
  api_capsule = export_capi(m, "fpy.request.crequest", &capi);
  if(!api_capsule)
    goto error;

#endif
  response_capi = import_capi("fpy.response.cresponse");
  if(!response_capi)
    goto error;
